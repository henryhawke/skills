...
Use RTK for supported read-oriented Git commands while investigating a repository: `rtk git status`, `rtk git diff`, `rtk git log`, `rtk git show`, `rtk git branch`, and `rtk git worktree`. Run native Git for interactive or unsupported operations such as rebase, bisect, and cherry-pick; do not alter their semantics merely to route them through RTK.
